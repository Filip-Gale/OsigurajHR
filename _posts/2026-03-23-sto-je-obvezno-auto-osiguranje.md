---
layout: post
title: "Što je obvezno auto osiguranje (AO) u Hrvatskoj?"
description: "Obvezno auto osiguranje (AO) u Hrvatskoj — što pokriva, što ne pokriva, koliko košta i gdje ga kupiti. Kompletni vodič za 2026."
permalink: /blog/sto-je-obvezno-auto-osiguranje/
date: 2026-03-23
modified: 2026-03-23
image: /img/blog/sto-je-obvezno-ao.jpg
keywords: obvezno auto osiguranje, AO osiguranje, auto osiguranje Hrvatska, osiguranje od automobilske odgovornosti, cijena auto osiguranja
---

# Što je obvezno auto osiguranje (AO) u Hrvatskoj?

**Obvezno auto osiguranje (AO)**, službeno osiguranje od automobilske odgovornosti, zakonski je obavezno za svako registrirano vozilo u Hrvatskoj. Pokriva štetu koju vozač nanese trećim osobama — ali ne pokriva vlastitu štetu ni oštećenje vlastitog vozila.

<div class="stat-grid">
<div class="stat-card"><span class="stat-number">100%</span><span class="stat-label">Digitalna polica — odmah vidljiva na STP</span></div>
<div class="stat-card"><span class="stat-number">50–80 €</span><span class="stat-label">Moguća ušteda usporedbom</span></div>
<div class="stat-card"><span class="stat-number">664 €</span><span class="stat-label">Max. kazna za vožnju bez AO</span></div>
<div class="stat-card"><span class="stat-number">20 god.</span><span class="stat-label">Osiguraj.hr — HANFA licenca</span></div>
</div>

---

## Što AO pokriva?

Obvezno auto osiguranje isplaćuje štetu **trećim osobama** koje ste ozlijedili ili kojima ste oštetili imovinu u prometnoj nezgodi:

- **Tjelesne ozljede** trećih osoba — do 1.300.000 € po nezgodi
- **Materijalna šteta** na tuđoj imovini — do 1.300.000 € po nezgodi
- **Troškovi liječenja** ozlijeđenih putnika i pješaka

---

## Što AO NE pokriva?

| Situacija | Pokriva AO? | Rješenje |
|---|---|---|
| Oštećenje vašeg automobila u nezgodi | ❌ Ne | Kasko osiguranje |
| Krađa vozila | ❌ Ne | Kasko osiguranje |
| Tuča, poplava, požar | ❌ Ne | Kasko osiguranje |
| Tjelesna ozljeda vozača krivca | ❌ Ne | Nezgoda putnika (dodatak) |
| Šteta na tuđem vozilu | ✅ Da | — |
| Ozljede putnika u drugom vozilu | ✅ Da | — |

---

## Koliko košta AO u 2026.?

Cijena AO police ovisi o četiri glavna faktora:

### 1. Zona registracije
Hrvatska je podijeljena u zone prema frekvenciji prometnih nezgoda. Zagreb i Rijeka su skuplje zone:

| Zona | Primjer gradova | Orijentacijska cijena* |
|---|---|---|
| Zona 1 | Slavonija, Koprivnica | 110–160 € |
| Zona 3 | Zadar, Pula, Karlovac | 180–260 € |
| Zona 5 | Šibenik, Varaždin | 190–280 € |
| Zona 7 | Zagreb | 220–350 € |

*Za vozilo 55–85 kW, B4 razred bonusa, vozač 30–50 god.

### 2. Bonus/malus razred
Svaka godina bez štete donosi bonus — popust koji raste do **50% pri B10 razredu**. Promjenom osiguravatelja bonus se ne gubi — prenosi se putem OIB-a.

### 3. Starost vozača
Vozači do 25 godina plaćaju **30–50% više** zbog statistički više stope nezgoda.

### 4. Snaga motora (kW)
Viša snaga motora = viša premija. Razlika između 55 kW i 150 kW vozila može biti 40–60 € godišnje.

<div class="cta-inline">
<h3>Koliko košta AO za vaše vozilo?</h3>
<p>Unesite registarsku oznaku — kalkulator istovremeno prikazuje ponude vodećih osiguravatelja i pronalazi najnižu cijenu za vaš profil.</p>
<a href="/izracunaj-cijenu/" class="cta-btn-yellow">Izračunaj cijenu →</a>
</div>

---

## AO vs. kasko: koja je razlika?

| | Obvezno AO | Kasko |
|---|---|---|
| **Zakonska obaveza** | ✅ Obavezno | ❌ Nije obavezno |
| **Pokriva štetu na tuđem vozilu** | ✅ Da | ✅ Da |
| **Pokriva štetu na vašem vozilu** | ❌ Ne | ✅ Da |
| **Pokriva krađu** | ❌ Ne | ✅ Da |
| **Pokriva elementarne nepogode** | ❌ Ne | ✅ Da |
| **Cijena** | 110–350 €/god | 200–800 €/god |

**Preporuka:** Kasko se isplati za vozila novija od 5–7 godina ili vrijednija od 10.000 €.

---

## Gdje kupiti obvezno auto osiguranje?

Policu možete kupiti:

1. **Online putem usporednog portala** (Osiguraj.hr) — kalkulator odjednom prikazuje ponude više osiguravatelja, kupnja karticom, polica odmah
2. **Direktno kod jednog osiguravatelja** — dobivate samo jednu ponudu, bez usporedbe
3. **U stanici za tehnički pregled** — praktično, ali obično skuplje jer nema usporedbe
4. **Putem agenta/brokera** — sporo, nije online, nema trenutačne usporedbe

> **Savjet:** Usporedba ponuda više osiguravatelja može uštedjeti **50–80 € godišnje** na istoj razini pokrića. Bonus/malus razred se prenosi bez gubitka.

---

## Česta pitanja

### Je li obvezno osiguranje isto kao "zelena karta"?

Zelena karta (međunarodna potvrda o osiguranju) dokaz je da vaše vozilo ima važeće obvezno AO osiguranje u državama koje su potpisnice Zelene karte konvencije. U svakodnevnoj vožnji u Hrvatskoj nije obavezna — potrebna je za međunarodne putove.

### Što se dogodi ako vozim bez osiguranja?

Vožnja bez obveznog auto osiguranja kažnjava se novčanom kaznom od **66,36 € do 664,00 €**. Uz kaznu, snosite osobnu financijsku odgovornost za svu štetu nanesenu trećima — bez gornje granice.

### Mogu li kupiti AO samo za nekoliko dana?

Da, moguće je kupiti kratkoročnu policu (npr. za privremeno korištenje vozila). Minimalno trajanje ovisi o osiguravatelju — obično je 30 dana. Dnevne police za putovanje u inozemstvo dostupne su kod pojedinih osiguravatelja.

### Prenosi li se polica na novog vlasnika pri prodaji vozila?

Ne. Polica je vezana uz vlasnika (OIB), ne uz vozilo. Novi vlasnik mora kupiti vlastitu policu. Prethodna polica se raskida, a eventualni višak premije vraća se dotadašnjem vlasniku.

---

<div class="cta-inline">
<h3>Usporedite ponude u 30 sekundi</h3>
<p>Bez obveze, bez poziva. Polica aktivna odmah — vidljiva na svim STP stanicama u cijeloj Hrvatskoj.</p>
<a href="/izracunaj-cijenu/" class="cta-btn-yellow">Usporedi osiguravatelje →</a>
</div>

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "FAQPage",
  "mainEntity": [
    {
      "@type": "Question",
      "name": "Što je obvezno auto osiguranje?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Obvezno auto osiguranje (AO), službeno osiguranje od automobilske odgovornosti, zakonski je obavezno za svako registrirano vozilo u Hrvatskoj. Pokriva štetu koju vozač nanese trećim osobama — tjelesne ozljede i materijalnu štetu. Ne pokriva vlastitu štetu vozača ni oštećenje vlastitog vozila."
      }
    },
    {
      "@type": "Question",
      "name": "Što pokriva obvezno auto osiguranje?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Obvezno auto osiguranje pokriva: tjelesne ozljede trećih osoba (do 1.300.000 € po nezgodi), materijalnu štetu na tuđoj imovini (do 1.300.000 € po nezgodi) i troškove liječenja ozlijeđenih. Ne pokriva: vlastitu materijalnu štetu na vozilu, krađu vozila, elementarne nepogode ni tjelesne ozljede samog vozača. Za ta pokrića potrebno je kasko osiguranje."
      }
    },
    {
      "@type": "Question",
      "name": "Koliko košta obvezno auto osiguranje u Hrvatskoj?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Cijena obveznog auto osiguranja u Hrvatskoj ovisi o zoni registracije, snazi motora, bonusu i starosti vozača. Orijentacijski rasponi za 2026: zona 1 (Slavonija) 110–160 €, zona 3 (Zadar, Pula) 180–260 €, zona 7 (Zagreb) 220–350 € godišnje za prosječno vozilo. Usporedbom više osiguravatelja moguće je uštedjeti 50–80 € godišnje."
      }
    },
    {
      "@type": "Question",
      "name": "Mora li svaki automobil imati auto osiguranje?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Da. Svako motorno vozilo koje se koristi na javnim prometnicama u Hrvatskoj mora imati važeće obvezno auto osiguranje. Iznimka su vozila trajno isključena iz prometa (odjavljena vozila). Vožnja bez važeće police kažnjava se novčanom kaznom od 66,36 € do 664,00 €."
      }
    },
    {
      "@type": "Question",
      "name": "Koja je razlika između AO i kasko osiguranja?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "AO (obvezno auto osiguranje) pokriva štetu koju nanesete drugima i zakonski je obavezno. Kasko osiguranje pokriva štetu na vašem vlastitom vozilu (nezgoda, krađa, tuča, elementarne nepogode) i nije obavezno. Kasko se preporuča za novija i skuplja vozila."
      }
    }
  ]
}
</script>
